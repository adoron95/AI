...
def hdistance1(s):                   # for h1 (tiles out of place)
    sum=0
    # compare between good list and bad list
...
def hdistance2(s):                   # for h2 (Manhattan)
    ...
    for i in range(len(s[0])):
        if i != s[0][i]:
            colTar = i%n                       # column target
            lineTar = i//n                     # line target
            colRial = s[0][i] % n              # column riality
            lineRial = s[0][i] //n             # line riality
            # Manhattan range is absolute value between the lines and the culomns
            count+= abs(colRial-colTar) + abs(lineRial-lineTar)
    return count
